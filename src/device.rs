...
pub async fn connect(candidate: &CandidateDevice) -> Result<Device, MirajazzError> {
    // Some N3 batches send both down/up states but ship with a VID/PID that otherwise maps to the
    // older protocol version. Force protocol v3 so `state` byte is respected and we don't emit
    // Down+Up in the same tick.
    let protocol_version = if candidate.dev.vendor_id == MIRABOX_VID && candidate.dev.product_id == N3_PID {
        3
    } else {
